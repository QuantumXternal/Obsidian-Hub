...
local Z= "CNMJ_FlightSpeed.txt"
local z= "CNMJ_EggSelectConfig.json"
...
local function T(...)
    ...
    if type(e)~= "table" then
        ...
        [ "performanceMode" ]= false ,[ "disable3D" ]= false ,[ "antiAFK" ]= true ,[ "returnHeight" ]= 70 ,[ "godmode" ]= false ,[ "antiKnockback" ]= false ,[ "antiTrap" ]= false ,[ "language" ]= "EN" }
    else
        ...
        if e.antiAFK ==nil then e.antiAFK = true end if e.returnHeight ==nil then e.returnHeight = 70 end if e.godmode ==nil then e.godmode = false end if e.antiKnockback ==nil then e.antiKnockback = false end if e.antiTrap ==nil then e.antiTrap = false end
...
local function x(...) pcall(function(...)
        if writefile and(a and h)then
            ...
            [ "godmode" ]=(h.godmode == true ),[ "antiKnockback" ]=(h.antiKnockback == true ),[ "antiTrap" ]=(h.antiTrap == true ),[ "antiAFK" ]=(h.antiAFK == true ),[ "returnHeight" ]=math.clamp (tonumber(h.returnHeight )or 70 , 10 , 100 );
...
local W=T()h={[ "godmode" ]=(W.godmode == true ),[ "antiKnockback" ]=(W.antiKnockback == true ),[ "antiTrap" ]=(W.antiTrap == true ),[ "autoGlide" ]= true ,[ "autoHatch" ]= false ;
...
Z4=function(e,...) if not (h and h.antiKnockback) then return end
...
z4=function(e,...)
    ...
    e.DescendantAdded :Connect(function(y,...)
        if y:IsA( "BallSocketConstraint" )or y:IsA( "HingeConstraint" )or y:IsA( "NoCollisionConstraint" )then
            task.defer (function(...) if h and h.antiKnockback then pcall(function(...) y:Destroy() end) Z4(e) end end)
        elseif y:IsA( "LocalScript" )and((string.find (string.lower (y.Name ), "ragdoll" )or string.find (string.lower (y.Name ), "fall" )))then
            if h and h.antiKnockback then y.Disabled = true end
        end
    end
...
local function enableAntiKnockback() h.antiKnockback = true pcall(function(...) Z4(o.Character) end) pcall(function(...) z4(o.Character) end) end local function disableAntiKnockback() h.antiKnockback = false pcall(function(...) local e=o.Character local y=e and e:FindFirstChildOfClass("Humanoid") if y then y:SetStateEnabled(Enum.HumanoidStateType.Ragdoll , true )y:SetStateEnabled(Enum.HumanoidStateType.FallingDown , true ) end end) end local function disableDesyncGodmode()
    b4(false)
end
...
g4=function(e,r,u,...)
    ...
    while h.alive and(h.isReturning and os.clock ()<s)do
        ...
        local Q=e.Z +g
        local P=(h and h.antiTrap) and i4() or {}
        local N= false
        if e.X >E then
...
local function wk(e,r,u,w,...)
    ...
    while h.alive and(h.glidingToTarget and os.clock ()<s)do
        ...
        local D= false
        if o> 25 then
            local e=(h and h.antiTrap) and i4() or {}
...
local function Mk(...) h.performanceMode = true pcall(function(...)
        local e=r:FindFirstChild( "CNMJ_EggESP" )
        if e then
            e:Destroy()
        end
        ...
        if e then
            e:SendKeyEvent( true ,Enum.KeyCode.Escape , false ,game)task.wait ( 0.12 )e:SendKeyEvent( false ,Enum.KeyCode.Escape , false ,game)task.wait ( 0.35 )e:SendKeyEvent( true ,Enum.KeyCode.Escape , false ,game)task.wait ( 0.12 )e:SendKeyEvent( false ,Enum.KeyCode.Escape , false ,game)pcall(function(...)
                if typeof(e.SendTouchEvent )== "function" then
                    e:SendTouchEvent( 99999 , 0 , 15 , 15 )task.wait ( 0.10 +math.random ()* 0.08 )e:SendTouchEvent( 99999 , 2 , 15 , 15 )
                end
            end
            )
        end
...
local lastHU4=0 y.Heartbeat :Connect(function(...)
    ...
    if y and not((h and h.onTreadmill )) and (h and h.antiKnockback) then
...
local function rM(e,r,y,...) pcall(function(...)
        ...
        if u and u:IsA( "GuiObject" )then
            ...
            local j=u:FindFirstChild( "CNMJAccentStroke" )or u:FindFirstChildOfClass( "UIStroke" )
            if j then
                j:Destroy()
            end
...
local function yM(...) rM(Fk.togTween ,Color3.fromRGB(184, 67, 95),Color3.fromRGB(46, 24, 32))rM(Fk.togTeleport ,Color3.fromRGB(211, 92, 122),Color3.fromRGB(46, 28, 36))rM(Fk.btnPlaceEgg ,Color3.fromRGB(160, 55, 85),Color3.fromRGB(42, 24, 32))rM(Fk.togAutoPlaceEvery5 ,Color3.fromRGB(195, 75, 105),Color3.fromRGB(46, 26, 34))rM(Fk.togGodmode ,Color3.fromRGB(184, 67, 95),Color3.fromRGB(46, 24, 32))rM(Fk.togAntiKnockback ,Color3.fromRGB(184, 67, 95),Color3.fromRGB(46, 24, 32))rM(Fk.togAntiTrap ,Color3.fromRGB(184, 67, 95),Color3.fromRGB(46, 24, 32))rM(Fk.btnUnstick ,Color3.fromRGB(175, 80, 95),Color3.fromRGB(44, 26, 32))rM(Fk.btnReset ,Color3.fromRGB(145, 48, 73),Color3.fromRGB(38, 22, 30))rM(Fk.btnLangSettings ,Color3.fromRGB(205, 100, 120),Color3.fromRGB(46, 28, 34))
end
local function uM(e,...)
...
local function jM(...)
    ...
    local H=Instance.new ( "UIGradient" ,V)H.Color =ColorSequence.new ({ColorSequenceKeypoint.new ( 0 ,Color3.fromRGB(184, 67, 95)),ColorSequenceKeypoint.new ( 1 ,Color3.fromRGB(211, 92, 122))})
    local t=Instance.new ( "TextLabel" )t.Size =UDim2.new ( 1 , -28 , 0 , 16 )t.Position =UDim2.new ( 0 , 14 , 0 , 94 )t.BackgroundTransparency = 1 t.Text = "Initializing Cheat Ni Mong - Jay..." t.TextColor3 =Color3.fromRGB(168, 155, 160)t.TextSize = 11 t.Font =Enum.Font.Gotham t.TextXAlignment =Enum.TextXAlignment.Left t.AutoLocalize = false t.Parent =r task.spawn (function(...)
        for y= 1 , 100 , 1 do
            if not e.Parent then
                break
            end
...
local kM={}kM.Gui =Instance.new ( "ScreenGui" )kM.Gui.Name = "CNMJ_RESTORE_BAR" kM.Gui.ResetOnSpawn = false kM.Gui.DisplayOrder = 999999 kM.Gui.ZIndexBehavior =Enum.ZIndexBehavior.Sibling kM.Gui.AutoLocalize = false pcall(function(...)
    if syn and syn.protect_gui then
...
local function aM(...) h.alive = false pcall(Ik)pcall(Ak)pcall(function(...) y:Set3dRenderingEnabled( true )
        ...
        local y=r:FindFirstChild( "CNMJ_EggESP" )
        if y then
            y:Destroy()
        end
    end
    ...
    pcall(function(...)
        for r,y in ipairs(game.CoreGui :GetChildren())do
            if y.Name :find( "CNMJ_" )or y.Name :find( "Dice_" )or y.Name :find( "DesyncSniperUI" )or y.Name :find( "WindUI" )then
                y:Destroy()
            end
        end
    end
...
local function oM(...)
    ...
    local function j(e,...)
        ...
        if not y then
            pcall(function(...)
                (game:GetService( "StarterGui" )):SetCore( "SendNotification" ,{[ "Title" ]=tostring(e.Title or "Cheat Ni Mong - Jay" ),[ "Text" ]=tostring(e.Content or "" );
                [ "Duration" ]= 3 })
            end
            )
        end
    ...
    if r then
        pcall(function(...)
            ...
            if e then
                r.Notify =function(r,y,...)
                    ...
                    if not u then
                        pcall(function(...)
                            (game:GetService( "StarterGui" )):SetCore( "SendNotification" ,{[ "Title" ]=tostring(y and y.Title or "Cheat Ni Mong - Jay" );
        ...
        local p=r:CreateWindow({[ "Title" ]= "Cheat Ni Mong - Jay" ;
        ...
        local function R(...)
            if p.UIElements and p.UIElements.Main then
                (u:Create(kM.Btn ,TweenInfo.new ( 0.12 ,Enum.EasingStyle.Quart ),{[ "Size" ]=UDim2.fromOffset ( 42 , 42 )})):Play()task.wait ( 0.08 +math.random ()* 0.06 )kM.Btn.Size =UDim2.fromOffset ( 46 , 46 )p.UIElements.Main .Visible = true kM.Btn.Visible = false
                if p.TabModule and p.TabModule.SelectedTab then
                    pcall(function(...) p.TabModule :SelectTab(p.TabModule.SelectedTab )
                    end
                    )
        ...
        })Fk.secSafety =Yk:Section({[ "Title" ]=P.Character.SecSafety })Fk.togGodmode =Yk:Toggle({[ "Title" ]= "Godmode" ;
        [ "Desc" ]= "Health protection only - you cannot die. Nothing else." ,[ "Icon" ]= "solar:shield-check-bold" ,[ "Value" ]=(h.godmode == true ) ;
        [ "Callback" ]=function(e,...)
            if e then
                enableDesyncGodmode()j({[ "Title" ]= "Godmode" ,[ "Content" ]= "Godmode enabled (health only)" ,[ "Icon" ]= "shield-check" })
            else
                disableDesyncGodmode()j({[ "Title" ]= "Godmode" ,[ "Content" ]= "Godmode disabled" ,[ "Icon" ]= "shield-off" })
            end
            x()
        end
        })Fk.togAntiKnockback =Yk:Toggle({[ "Title" ]= "Anti Knockback" ;
        [ "Desc" ]= "No ragdoll, no flying from monsters. Health and traps untouched." ,[ "Icon" ]= "solar:shield-check-bold" ,[ "Value" ]=(h.antiKnockback == true ) ;
        [ "Callback" ]=function(e,...)
            if e then
                enableAntiKnockback()j({[ "Title" ]= "Anti Knockback" ,[ "Content" ]= "Anti Knockback enabled" ,[ "Icon" ]= "shield-check" })
            else
                disableAntiKnockback()j({[ "Title" ]= "Anti Knockback" ,[ "Content" ]= "Anti Knockback disabled" ,[ "Icon" ]= "shield-off" })
            end
            x()
        end
        })Fk.togAntiTrap =Yk:Toggle({[ "Title" ]= "Anti Trap" ;
        [ "Desc" ]= "Dodge hitboxes while flying and safety pad on landing." ,[ "Icon" ]= "solar:shield-check-bold" ,[ "Value" ]=(h.antiTrap == true ) ;
        [ "Callback" ]=function(e,...)
            if e then
                h.antiTrap = true j({[ "Title" ]= "Anti Trap" ,[ "Content" ]= "Anti Trap enabled (dodge + pad)" ,[ "Icon" ]= "shield-check" })
            else
                h.antiTrap = false pcall(function(...) if _G.CNMJ_Pad then _G.CNMJ_Pad:Destroy() _G.CNMJ_Pad =nil end end)j({[ "Title" ]= "Anti Trap" ,[ "Content" ]= "Anti Trap disabled" ,[ "Icon" ]= "shield-off" })
            end
            x()
        end
    ...
    local l=Instance.new ( "TextLabel" )l.Size =UDim2.new ( 1 , -90 , 0 , 22 )l.Position =UDim2.new ( 0 , 12 , 0 , 6 )l.BackgroundTransparency = 1 l.Text = "Cheat Ni Mong - Jay (Fallback)" l.TextColor3 =B l.TextSize = 14 l.Font =Enum.Font.GothamBold l.TextXAlignment =Enum.TextXAlignment.Left l.AutoLocalize = false l.Parent =U
    local D=Instance.new ( "TextLabel" )D.Size =UDim2.new ( 1 , -90 , 0 , 14 )D.Position =UDim2.new ( 0 , 12 , 0 , 26 )D.BackgroundTransparency = 1 D.Text = "Steal an Egg v42.64" D.TextColor3 =Color3.fromRGB(232, 160, 181)D.TextSize = 11 D.Font =Enum.Font.Gotham D.TextXAlignment =Enum.TextXAlignment.Left D.AutoLocalize = false D.Parent =U
    ...
    local q=Instance.new ( "TextButton" )q.Size =UDim2.new ( 0 , 28 , 0 , 28 )q.Position =UDim2.new ( 1 , -36 , 0 , 9 )q.BackgroundColor3 =Color3.fromRGB(145, 48, 73)q.Text = "X" q.TextColor3 =B q.TextSize = 12 q.Font =Enum.Font.GothamBold q.AutoButtonColor = false q.Parent =U;
    ...
    local function E(e,r,...)
        ...
        local u=Instance.new ( "TextLabel" )u.Size =UDim2.new ( 1 , 0 , 1 , 0 )u.BackgroundTransparency = 1 u.Text =e u.TextColor3 =Color3.fromRGB(211, 92, 122)u.TextSize = 11 u.Font =Enum.Font.GothamBold u.TextXAlignment =Enum.TextXAlignment.Left u.AutoLocalize = false u.Parent =y
        return y
    end
    ...
    local S= false
    local Z=nil
    local z=nil Z=b( "Auto Steal (Tween)" , "Fly to steal eggs & auto stash into backpack" ,h.pureTweenFarm ,Color3.fromRGB(184, 67, 95), 11 ,function(e,...)
        if S then
            return
        end
        if e then
    ...
    )z=b( "Auto Steal (Teleport)" , "Teleport to steal eggs in continuous loop" ,h.autoFarmLoop ,Color3.fromRGB(211, 92, 122), 12 ,function(e,...)
        if S then
            return
        end
        if e then
    ...
    A( "Single Steal (Teleport)" , "Teleport to steal 1 target egg and return" ,Color3.fromRGB(184, 67, 95), 13 ,function(...) task.spawn (function(...)
            if Y4~= "NONE" then
    ...
    )E( "PLACE EGG" , 20 )A( "Place Egg" , "Tween home, place all carried eggs & hatch" ,Color3.fromRGB(160, 55, 85), 21 ,function(...) task.spawn (function(...) h.statusText = "[Manual] Depositing eggs..." g4(h.glideSpeed )v4()u4()h.isReturning = false h.delivering = false
        end
        )
    end
    )b( "Auto Place (Every 5)" , "Return home every 5 steals, place & wait 5s" ,h.autoPlaceEvery5 ,Color3.fromRGB(195, 75, 105), 22 ,function(e,...) h.autoPlaceEvery5 =e
        if not e then
            h.batchStealCount = 0
        end
    end
    )b( "Auto Hatch" , "Automatically hatch ready eggs continuously" ,h.autoHatch ,Color3.fromRGB(184, 67, 95), 22 ,function(e,...) h.autoHatch =e
    end
    ...
    )E( "CHARACTER & SAFETY" , 30 )b( "Godmode" , "Health only - you cannot die" , (h.godmode == true ) ,Color3.fromRGB(184, 67, 95), 31 ,function(e,...)
        if e then
            enableDesyncGodmode()
        else
    ...
    )b( "Anti Knockback" , "No ragdoll, no flying from monsters" , (h.antiKnockback == true ) ,Color3.fromRGB(184, 67, 95), 32 ,function(e,...)
        if e then
            enableAntiKnockback()
        else
    ...
    )b( "Anti Trap" , "Dodge hitboxes while flying and pad on landing" , (h.antiTrap == true ) ,Color3.fromRGB(184, 67, 95), 33 ,function(e,...)
        if e then
            h.antiTrap = true
        else
            h.antiTrap = false pcall(function(...) if _G.CNMJ_Pad then _G.CNMJ_Pad:Destroy() _G.CNMJ_Pad =nil end end)
        end
        x()
    end
    )A( "Get Out Treadmill" , "Instantly escape from treadmill or gear" ,Color3.fromRGB(175, 80, 95), 34 ,function(...) pcall(M4)pcall(C4)pcall(D4)
    end
    ...
    local T=Instance.new ( "TextLabel" )T.Size =UDim2.new ( 0 , 70 , 0 , 24 )T.Position =UDim2.new ( 1 , -80 , 0.5 , -12 )T.BackgroundColor3 =V T.Text =string.format ( "%d Studs/s" ,h.glideSpeed or 600 )T.TextColor3 =Color3.fromRGB(232, 160, 181)T.TextSize = 11 T.Font =Enum.Font.GothamBold T.AutoLocalize = false T.Parent =F;
...
H( "[+] Initializing Cheat Ni Mong - Jay x WindUI v42.64 (Steal an Egg Edition)..." )oM()task.spawn (function(...) task.wait ( 0.5 )A4()b4( false )C4()
    if o.Character then
        z4(o.Character )
    end
...
H( "[+] Cheat Ni Mong - Jay v42.64 Ready! All-In-One Built-in Anti-AFK & Prometheus Ready!" )
